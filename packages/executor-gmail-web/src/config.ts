...
const configSchema = z.object({
  ...
  maxProposalsPerRun: z.coerce.number().int().positive().max(10).default(1),
  actorId: z.string().trim().min(1).default("gmail-web-executor"),
  actorDisplayName: z.string().trim().min(1).default("Gmail Browser Executor"),
...
export function parseExecutorConfig(
  ...
  return configSchema.parse({
    ...
    maxProposalsPerRun: env.GMAIL_EXECUTOR_MAX_PROPOSALS_PER_RUN ?? 1,
    actorId: env.GMAIL_EXECUTOR_ACTOR_ID ?? "gmail-web-executor",
    actorDisplayName: env.GMAIL_EXECUTOR_ACTOR_DISPLAY_NAME ?? "Gmail Browser Executor",
